<template>
	<div class="YearEndReview YearEndReview-rewards">
		<Sliders
			:numberOfScreens="10"
			:title="'Rewards'"
		...
		<div class="YearEndReview-rewards__content">
			<Heading
				type="h2"
				className="YearEndReview__h2 rewards"
			>
				Kudos sa ‘yo for maximizing your voucher savings!
				<span class="YearEndReview-rewards__span-amount">PHP {{ dataConvert.rewardsTotal }}</span>
			</Heading>
			<div class="YearEndReview-rewards__content-details">
				<div class="preamble">Heto ang mga fave mong vouchers!</div>
				<div class="cashback-row row-1">
					<div class="block-wrap">
						<div class="block-title">Cashback</div>
						<div class="block-amount">{{ dataConvert.cashback }}</div>
					</div>
				</div>
				<div class="cashback-row row-2">
					<div class="a-rewards">
						<div class="block">
							<div class="block-wrap">
								<div class="block-title">A+ Rewards</div>
								<div class="block-amount">{{ dataConvert.aRewards }}</div>
							</div>
						</div>
					</div>
					<div class="vouchers">
						<div class="block">
							<div class="block-title">Vouchers</div>
							<div class="block-amount">{{ dataConvert.vouchers }}</div>
						</div>
					</div>
				</div>
			</div>
...
<script>
import './style.less';
import { Heading, Sliders, Button, ShareFourCol } from '../../components';
...
export default {
	...
	computed: {
		dataConvert() {
			...
			return {
				rewardsTotal: numberWithCommas(data['rewards-total']),
				cashback: numberWithCommas(data['cashback']),
				aRewards: numberWithCommas(data['a-rewards']),
				vouchers: numberWithCommas(data['vouchers']),
			};
		},
	},
	mixins: [sharePreview],
