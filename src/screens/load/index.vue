<template>
	<div class="YearEndReview YearEndReview-buy-load">
		<Sliders
			:numberOfScreens="10"
			:title="'Load'"
			...
			:color="'#FFFFFF'"
		/>
		<div class="YearEndReview-buy-load__wrapper">
			<div class="YearEndReview-buy-load__details">
				<div class="YearEndReview-buy-load__details-top1">
					<img
						:src="require('../../assets/images/plain/load-circular.png')"
						alt=""
					/>
					<div class="favorite-load">
						Your favorite Load is <br />
						<span>{{ dataConvert.favoriteLoad[0].name }}</span>
					</div>
				</div>
				<div class="YearEndReview-buy-load__details-others">
					<div>These are the runner-ups!</div>

					<ol start="2">
						<li
							v-for="(item, index) in dataConvert.runnerUps"
							:key="index"
							class="load-runnerups"
						>
							{{ item.name }}
						</li>
					</ol>
		...
		<div class="YearEndReview-buy-load__content">
			<Heading
				type="h2"
				className="YearEndReview__h2 load "
			>
				Wow! Todo ang bonding sa <br />
				dami ng Buy Load mo!
				<span class="YearEndReview-send__span-amount">PHP {{ dataConvert.buyLoadTotal }}</span>
			</Heading>
		</div>
...
<script>
import './style.less';
import { Heading, Sliders, ShareTwoCol, Button, ShareFourCol } from '../../components';
...
export default {
	...
	computed: {
		dataConvert() {
			...
			return {
				buyLoadTotal: numberWithCommas(data['buy-load-total']),
				favoriteLoad: data['favorite-Load'],
				runnerUps: data['favorite-Load'].slice(1),
			};
		},
	},
	mixins: [sharePreview],
