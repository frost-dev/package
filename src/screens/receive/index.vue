<template>
	<div class="YearEndReview-receive">
		<Sliders
			:numberOfScreens="10"
			:title="'Receive'"
			...
			:color="'#FFFFFF'"
		/>
		<div class="YearEndReview-receive__content">
			<Heading
				type="h2"
				className="YearEndReview__h2 receive"
			>
				Basta <span class="YearEndReview-receive__span-name">Receive Money,</span>
				<span class="YearEndReview-receive__span-thankful"> lagi kang #Thankful! </span>
				<span class="YearEndReview-receive__span-amount">PHP {{ dataConvert.amount }}</span>
			</Heading>
			<div class="YearEndReview-receive__description">
				Received <span>{{ dataConvert.frequency }} Times </span> from <span>{{ dataConvert.users }} users</span>
			</div>
		</div>
		...
		>
            <ShareTwoCol
                ref="yearEndReview"
                title="Receive"
                description="Deserve mo lahat ng Receive Money mo this year!"
                :blocks="{
                    frequency: dataConvert.frequency,
                    users: dataConvert.users,
                }"
                type="Receive"
                bg="background: linear-gradient(0deg, #0081E5, #0081E5), linear-gradient(360deg, rgba(0, 255, 178, 0) 53.91%, rgba(0, 255, 178, 0.4) 100%);"
            />
		</div>
	</div>
</template>
...
export default {
	...
	computed: {
		dataConvert() {
			...
			return {
				amount: this.numberWithCommas(data['receive-money-total']),
				frequency: data['receive-money-frequency'],
				users: data['receive-money-users'],
			};
		},
	},
	mixins: [sharePreview],
