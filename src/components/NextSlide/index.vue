...
export default {
	...
	mounted() {
		this.$nextTick(() => {
			let nextWrapper = this.$refs.nextWrapperRef;
			let clickTarget = nextWrapper.querySelector(this.clickTarget);

			if (nextWrapper && clickTarget) {
				nextWrapper.addEventListener('touchstart', this.handleTouchStart);
				nextWrapper.addEventListener('touchmove', this.handleTouchMove);
				nextWrapper.addEventListener('touchend', this.handleSwipeDown);
				clickTarget.addEventListener('click', this.toPrevSlide);
			}
		});
	},
	methods: {
